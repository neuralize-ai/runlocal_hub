...
class JobStatusDisplay:
    ...
    def create_status_table(self, job_results: List[JobResult], job_type: JobType) -> Table:
        ...
        for result in job_results:
            status_text = Text(result.status.value, style=StatusColors.get_color(result.status))
            ...
            if result.status == BenchmarkStatus.Failed and result.error:
                details = Text(result.error[:50] + "..." if len(result.error) > 50 else result.error, style="red")

...
def _convert_requests_error(requests_error: requests.exceptions.RequestException, function_name: str) -> RunLocalError:
    """
    ...
    if isinstance(requests_error, requests.exceptions.ConnectionError):
        return NetworkError(
            ...
            endpoint=getattr(requests_error.request, 'url', None) if hasattr(requests_error, 'request') else None
        )
        ...
        return NetworkError(
            f"Network error in {function_name}: {str(requests_error)}"
        )
